public class Projection3D extends Projection {
    ...
    /*private final static double _2PI = 2 * Math.PI;
     ...
     private final static double[] COS = DoubleArray.f(DoubleArray.increment(N,
     0, 2 * Math.PI / (N - 1)), new Function() {
     public double f(double x) {
     return Math.cos(x);
     }
     });

     private final static double[] SIN = DoubleArray.f(DoubleArray.increment(N,
     0, 2 * Math.PI / (N - 1)), new Function() {
     public double f(double x) {
     return Math.sin(x);
     }
